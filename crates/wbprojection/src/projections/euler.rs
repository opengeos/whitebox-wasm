    ...
    pub fn new(p: &ProjectionParams, lat1: f64, lat2: f64) -> Result<Self> {
        ...
        if del.abs() < EPS || sig.abs() < EPS {
            return Err(ProjectionError::InvalidParameter {
                ...
                reason: "Euler requires |lat1-lat2|>0 and |lat1+lat2|>0".to_string(),
            });
        }

        let n = sig.sin() * del.sin() / del;
        let del_h = del * 0.5;
        let rho_c = del_h / (del_h.tan() * sig.tan()) + sig;
...
impl ProjectionImpl for EulerProj {
