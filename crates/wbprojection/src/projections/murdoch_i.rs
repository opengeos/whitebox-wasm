    ...
    pub fn new(p: &ProjectionParams, lat1: f64, lat2: f64) -> Result<Self> {
        ...
        if del.abs() < EPS || sig.abs() < EPS {
            return Err(ProjectionError::InvalidParameter {
                ...
                reason: "Murdoch I requires |lat1-lat2|>0 and |lat1+lat2|>0".to_string(),
            });
        }

        let rho_c = del.sin() / (del * sig.tan()) + sig;
...
impl ProjectionImpl for MurdochIProj {
