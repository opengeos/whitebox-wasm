...
pub(super) struct MurdochIIIProj {
    lon0: f64,
    a: f64,
    fe: f64,
    fn_: f64,
    ...
    pub fn new(p: &ProjectionParams, lat1: f64, lat2: f64) -> Result<Self> {
        ...
        if del.abs() < EPS || sig.abs() < EPS {
            return Err(ProjectionError::InvalidParameter {
                ...
                reason: "Murdoch III requires |lat1-lat2|>0 and |lat1+lat2|>0".to_string(),
            });
        }

        let rho_c = del / (sig.tan() * del.tan()) + sig;
        let rho_0 = rho_c - lat0;
        let n = sig.sin() * del.sin() * del.tan() / (del * del);
...
impl ProjectionImpl for MurdochIIIProj {
