...
pub(super) struct VitkovskyIProj {
    lon0: f64,
    a: f64,
    fe: f64,
    fn_: f64,
    ...
    pub fn new(p: &ProjectionParams, lat1: f64, lat2: f64) -> Result<Self> {
        ...
        if del.abs() < EPS || sig.abs() < EPS {
            return Err(ProjectionError::InvalidParameter {
                ...
                reason: "Vitkovsky I requires |lat1-lat2|>0 and |lat1+lat2|>0".to_string(),
            });
        }

        let cs = del.tan();
        let n = cs * sig.sin() / del;
        let rho_c = del / (cs * sig.tan()) + sig;
...
impl ProjectionImpl for VitkovskyIProj {
